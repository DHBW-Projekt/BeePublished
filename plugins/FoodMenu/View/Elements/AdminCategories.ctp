<div id="adminCategory">
	...
	<?php 
	echo $this->Form->create('FoodMenuCategory', array('url' => array('plugin' => 'FoodMenu', 'controller' => 'FoodMenuApp', 'action' => 'deleteCategories')));
	...
	if (isset($categories)) {
		foreach ($categories as $category) {
			if ( $category['FoodMenuCategory']['deleted'] != NULL ) continue;
				...
				echo '</td><td>';
				echo $this->Html->image('/app/webroot/img/edit.png', array('onClick' => 'showDiv(\'adminCategoryEdit\', \'adminCategoryOverview\')', 'style' => 'float: left', 'width' => '20px', 'alt' => '[e]Edit', 'url' => array('plugin' => 'FoodMenu', 'controller' => 'FoodMenuApp', 'action' => 'editCategory', $category['FoodMenuCategory']['name'], $category['FoodMenuCategory']['id'])));
				echo '</td><td>';
