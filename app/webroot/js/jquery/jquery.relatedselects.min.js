...
(function($){
	...
	var RelatedSelect = function(context, opts) {
		var $context = $(context), selects = [], inputFields = [];
        ...
        for(var key in opts.inputFields){
		...
		function populate($caller,$select,o){
            ...
            for(var x=0, len=inputFields.length; x<len; x++){
				selectors.push('input[name="'+inputFields[x]+'"]');
			};
        ...
        function inputsToObj(){
			var arrSelects = opts.inputFields;
			opts.inputFields = {};
			for(var i=0, len=arrSelects.length; i<len; i++){
				opts.inputFields[ arrSelects[i] ] = {};
			};
		};
	};
