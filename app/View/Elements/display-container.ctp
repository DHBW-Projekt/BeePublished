...
<div class="subcolumns"><?php foreach ($child['columns'] as $column): ?>
<div class="<?php echo $column['class']; ?>">
<div class="<?php echo $column['contentClass']; ?>"><?php echo $this->element('display-container', array('data' => $column['children'])); ?>
</div>
</div>
<?php endforeach; ?></div>
...
<?php else: ?>
<div class="plugin_content"><?php echo $this->element($child['content']['view'], array('data' => $child['content']['viewData'], 'url' => $child['content']['pageUrl'], 'contentId' => $child['content']['id'], 'pluginId' => $child['content']['pluginId']), array('plugin' => $child['content']['plugin'])); ?>
<?php if($this->PermissionValidation->getUserRole() == 6 || $this->PermissionValidation->getUserRole() == 7): ?>
<div class="plugin_administration"><?php
