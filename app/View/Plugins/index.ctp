...
<table>
	...
	<?php foreach ($installed as $plugin): ?>
		<tr>
			...
			<td><?php if ($plugin['status'] == 0): ?> <?php echo __('No Database tables'); ?>
			<?php elseif ($plugin['status'] == 2): ?> <?php echo $this->Html->link(__('Update plugin'), array('action' => 'install', $plugin['Plugin']['name'])); ?>
			<?php elseif ($plugin['status'] == 3): ?> <?php echo __('Up to date'); ?>
			<?php endif; ?></td>
